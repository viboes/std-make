...
optional<X> emplace(std::experimental::type<optional<X>>, Args&& ...args)
{
  optional<X> res;
...
int main()
{
  ...
  {
    int v=1;
    boost::optional<A> x = std::experimental::emplace<boost::optional<A>>(v,v);
    BOOST_TEST(x->v == 2);
  }
  {
    boost::optional<int> x = std::experimental::emplace<boost::optional<int>>();
    BOOST_TEST_EQ(*x,  0);
  }
//  {
//    int v=0;
