...
template <class X>
future<typename std::decay<X>::type> make(std::experimental::tc<std::future>, X&& x)
{
...
future<X> make(std::experimental::c<future<X>>, Args&& ...args)
{
...
template <class X>
future<typename std::decay<X>::type> make(std::experimental::c<future<std::experimental::_t>>, X&& x)
{
  typedef typename std::decay<X>::type value_type;
  promise<value_type> p;
  p.set_value(std::forward<X>(x));
