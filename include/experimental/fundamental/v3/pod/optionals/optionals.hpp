...
struct index<std::tuple<T, Ts...>, T>
{
	enum { value = 0 };
};
template <class T, class U, class... Ts>
struct index<std::tuple<T, Ts...>, U>
{
	enum { value = 1 + index<std::tuple<Ts...>, U>::value };
};
template <class Tpl, class T>
struct contains;
template <class T>
...
template <class T, class... Ts>
struct contains<std::tuple<T, Ts...>, T>
{
	enum { value = true };
};
template <class T, class U, class... Ts>
struct contains<std::tuple<T, Ts...>, U>
{
	enum { value = contains<std::tuple<Ts...>, U>::value };
};

template <class Tpl, class T>
struct none_of;
template <class T>
struct none_of<std::tuple<>, T>
{
	enum { value = true };
};
template <class T, class... Ts>
struct none_of<std::tuple<T, Ts...>, T>
{
	enum { value = false };
};
template <class T, class U, class... Ts>
struct none_of<std::tuple<T, Ts...>, U>
{
	enum { value = none_of<std::tuple<Ts...>, U>::value };
};

template <class Tpl, class T>
struct contains_unique;
template <class T>
struct contains_unique<std::tuple<>, T>
{
	enum { value = false };
};
template <class T, class... Ts>
struct contains_unique<std::tuple<T, Ts...>, T> : none_of<std::tuple<Ts...>, T>
{
};
template <class T, class U, class... Ts>
struct contains_unique<std::tuple<T, Ts...>, U>
{
	enum { value = contains_unique<std::tuple<Ts...>, U>::value };
};
...
public:
	...
	template <class T, JASEL_REQUIRES(detail::contains_unique<Types, T>::value)>
	constexpr bool has_value() const noexcept
	{
	...
	/// @returns A const optional_ref instance that behaves as optional except that the storage is collocated in this optional.
	template <size_t I>
	...
	// fixme: this should work only if T is unique and one of the elements
	template <class T, JASEL_REQUIRES(detail::contains_unique<Types, T>::value)>
...
private:
	size_t mask; // todo: make mask type depend on the number of optionals and their alignment
	Types  tpl;
};
...
// in some way optional<Ts> has a similar observer interface as variant<Ts>, as each one of the Ts can be there or not.
template <size_t I, class... Ts>
constexpr auto get_if(optionals<Ts...> &opts) -> decltype(opts.template get_if<I>())
{
	return opts.template get_if<I>();
}
template <size_t I, class... Ts>
constexpr auto get_if(optionals<Ts...> const &opts) -> decltype(opts.template get_if<I>())
{
	return opts.template get_if<I>();
}

template <class T, class... Ts>
constexpr auto get_if(optionals<Ts...> &opts) -> decltype(opts.template get_if<T>())
{
	return opts.template get_if<T>();
}
template <class T, class... Ts>
constexpr auto get_if(optionals<Ts...> const &opts) -> decltype(opts.template get_if<T>())
{
	return opts.template get_if<T>();
}
