...
struct underlying_type 
{ 
    using type = remove_refcv_t<decltype(wrapping::underlying(declval<T>()))>; 
};
...
template <class E, when<is_enum_v<E>>>
    struct traits
    {
        template <class T>
            constexpr auto underlying(T && x)
            {
                return static_cast<std::underlying_type_t<T>>(forward<T>(x));
            }
...
We can simplify the proposal just by requiring an more explicit mapping and remove the minimal complete definition based on wrapping objects.
 
Do we want the wrapping facility or do we prefer to have a more specific mapping.

## Wrapping naming

If we want the wrapping facilities, next follows some alternatives

Alternatives to `wrapping::underlying`

* `wrapping::underlying(W) -> underlying_type_t<W>`

We have already that C++ enums have wrap in some way an underlying integral type.

So either this function is overloaded or we need to add a specialization for `wrapping::traits<Enum>::underlying`. 

**todo** Add customization for enums.
**todo** Rename `unwrap` by `underlying`.
