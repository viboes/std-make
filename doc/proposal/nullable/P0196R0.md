...
Not proposed by this paper, would be the possibility to lift `none`. This lifted
...
A different ongoing proposal [MAKEF] C++ generic factories would propose a `no_value` function so that we could do
...
## But how exactly is `none` defined?
...
A possible implementation could be based on the current definition of `std::experimental::nullopt_t` and `std::experimental::nullopt`.
...
Thanks to Tony Van Eerd and Titus Winters for helping me to improve globally the paper. Thanks to Agustín Bergé K-ballo for his useful comments.
