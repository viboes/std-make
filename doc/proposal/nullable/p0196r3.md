...
We could define a *PossiblyValued* type of classes that is more general than *Nullable* and see *Nullable* as a special case of *PossiblyValued* when the not-a-value is a unit type. However the constraint on the default constructor could not be covered by *PossiblyValued* if we would want `expected<T,E>` to be a *PossiblyValued* type.
