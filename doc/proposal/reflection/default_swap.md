...
I propose to (implicitly) supply default version of this operation, if needed. The meaning `swap` is to swap each member. 
...
We propose then a member-wise `swap` definition.
...
A swap expression is a particular case of a function call expression when the function names is `swap`.
...
**Add a Special non-member swap operations section after 13.6**
...
If no user-defined `swap` operation is provided for a class type (struct, class or union), and all of the following is true: 
...
then the compiler will declare a `swap` operation in the class namespace with the signature 

```c++
void swap(T&, T&) noexcept(see below);
```

### Explicitly defaulted `swap` non-member operation

The user may still force the generation of the implicitly declared `swap` operation with the keyword default. 
...
If the implicitly-declared `swap` operation is not deleted, it is defined (that is, a function body is generated and compiled) by the compiler if odr-used. For union types, the implicitly-defined `swap` operation do as the `std::swap`. For non-union class types (class and struct), the `swap` non-member operation performs full member-wise swap of the object's bases and non-static members, in their initialization order. 
