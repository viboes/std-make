...
otherwise, if `! bool(*this) and ! bool(rhs)`, assigns `move(rhs).get_unexpected()` to the contained value `unexpect`; 

otherwise, if `bool(*this) and ! bool(rhs)`, 

* destroys the contained value by calling `val->T::~T()`,
* initializes the contained value as if direct-non-list-initializing an object of type `unexpected_type<E>` with `move(rhs).get_unexpected()`; 
