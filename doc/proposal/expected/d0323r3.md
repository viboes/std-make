...
* Remove `make_expected` factory and make use of deduction guides.
* Remove `make_unexpected` factory and make use of deduction guides.
* Remove the helper functions and move them to a future *PossiblyValued* proposal.
...
* Add the wording for void specialization.
* Removed Future Work section.
 
Other changes proposed by the authors:

* **TODO** Set the default value for the `Error` parameter to `void`
* **TODO** Specialize `expected<T,void>`.
* **TODO** Define `expected<T,E>` constructor from `expected<T,void>` 
* **TODO** Specialize `bad_expected_access<void>`.
* **TODO** Make `bad_expected_access<void>` inherit from `bad_expected_access<void>`.
...
Because the first behavior might lead to buggy application, we won’t consider it in a first time. The handling is dependent of the underlying error type, we consider the `exception_ptr` and the `errc` types.
...
```c++
expected<int, errc> divide2(int i, int j)
{
...
`expected<T,E>` models a discriminated union of types `T` and `unexpected<E>`. `expected<T,E>` is viewed as a value of type `T` or value of type `unexpected<E>`, allocated in the same storage, along with the way of determining which of the two it is.
...
As `boost::variant<T,unexpected<E>>`, `expected<T,E>` ensures that it is never empty. All instances `v` of type `expected<T,E>` guarantee `v` has constructed content of one of the types `T` or `E`, even if an operation on `v` has previously failed.
...
If the latter appears too cumbersome, one can always use the deduction guide `expected` described below:

```c++
expected<int, errc> ei = expected(1);
```
...
An alternative will be to make it explicit and add a `expected_type<T>` (similar to `unexpected<E>` explicitly convertible from `T` and implicitly convertible to `expected<T,E>`. 
...
It has been considered that we don't need yet comparison operator between `expected` objects and less yet between mixed `expected` and `unexpected`.

Another proposal could consider to retake the previous comparison as defined in [P0323R2].
...
`expected<T,E>` is implicitly convertible from `T` and `unexpected<E>`, where `unexpected<E>` is explicitly convertible from `E`.
...
We decided in Toronto to don't have a default. 

Maybe we could have void as default and have a specialization of `expected<T,void>` that behaves as an `expected<T>` that is convertible to any `expected<T,E>`.
...
An alternative is to inherit `bad_expect_access<E>` from `bad_expect_access<void>` and default the `E` parameter to void.

This point was not discussed enough in Toronto. The authors consider that this should be a correct design.
...
This subclause describes class template `unexpected` that wraps objects intended as unexpected. This
...
inline namespace fundamentals_v3 {
	...
	// X.Z.12, Deduction guides
	template <class T>
		constexpr expected<decay_t<T>, void> expected_from_value(T&& e);
	
}}}
```

A program that necessitates the instantiation of template `expected<T,E>` with `T` for a reference type or for possibly cv-qualified types `in_place_t`, `unexpect_t` or `unexpected<E>` is ill-formed.
...
```c++
	...
	// X.Z.5, expected specialization for `void` `Error`.
	template <class T>
...
public:
   ...
   // alternatively
	constexpr expected(const expected<T, void>&);
	constexpr expected(expected<T, void>&&);
...
If `!bool(rhs)` initializes the contained value as if direct-non-list-initializing an object of type `unexpected<E> ` with the expression `unexpected(rhs.error())`.

*Postconditions*: `bool(rhs) == bool(*this)`.

*Throws*: Any exception thrown by the selected constructor of `T` or`unexpected<E>`.
...
If `!bool(rhs)` initializes the contained value as if direct-non-list-initializing an object of type `unexpected<E> ` with the expression `std::move(unexpected(rhs.error()))`.
...
*Remark*: If `unexpected<E>`’s selected constructor is a constexpr constructor, this constructor shall be a constexpr constructor.
...
*Remarks*: If unexpected<E>’s constructor selected for the initialization is a constexpr constructor, this constructor shall be a constexpr constructor.
This constructor shall not participate in overload resolution unless
...
*Effects*: If `is_trivially_destructible<T>::value != true and bool(*this)`, calls `val.~T()`.
If `is_trivially_destructible<E>::value != true and ! (bool(*this)`, calls `unexpect.~unexpected<E>() `.
...
otherwise, if `! bool(*this) and ! bool(rhs)`, assigns `unexpected(rhs.error())` to the contained value `unexpect`; 

otherwise, if `bool(*this) and ! bool(rhs)`, 

* destroys the contained value by calling `val.~T()`,
* initializes the contained value as if direct-non-list-initializing an object of type `unexpected<E>` with `unexpected(rhs.error())`; 
...
* move constructs a new `unexpected<E> tmp` on the stack from `this.get_unexpecte()` (which can't throw as `E` is nothrow-move-constructible), 
* destroys the contained value by calling `unexpect.~ unexpected<E>()`, 
...
otherwise, if `! bool(*this) and ! bool(rhs)`, move assign `unexpected(rhs.error())` to the contained value `unexpect`; 
...
If `! bool(*this), move assign `unexpected(rhs.error())` to the contained value `unexpect`; 
...
if `bool(*this)`, assigns the contained value `val` as if constructing an object of type `T` with the arguments `std::forward<Args>(args)...`; otherwise, destroys the contained value by calling `unexpect.~unexpected<E>() ` and initializes the contained value as if constructing an object of type `T` with the arguments `std::forward<Args>(args)...`.
...
```
*Effects*: if `bool(*this)`, assigns the contained value `val` as if constructing an object of type `T` with the arguments `il, std::forward<Args>(args)...`, otherwise destroys the contained value by calling `unexpect.~unexpected<E>()` and initializes the contained value as if constructing an object of type `T` with the arguments `il, std::forward<Args>(args)...`.
...
*Returns*: If `bool(x) != bool(y)`, `false`; otherwise if `bool(x) == false`, `unexpected(x.error()) == unexpected(y.error())`; otherwise `*x == *y`.

*Remarks*: Specializations of this function template, for which `*x == *y` and `unexpected(x.error()) == unexpected(y.error())` are core constant expression, shall be constexpr functions.    
...
*Returns*: If `bool(x) != bool(y)`, `true`; otherwise if `bool(x) == false`, `unexpected(x.error()) != unexpected(y.error())`; otherwise `*x != *y`.
