...
namespace std
{
  template <> struct future<_t> : experimental::meta::quote<future> {};
}
```
...
namespace std
{
  ...
  template <>
    struct default_delete<_t> : experimental::meta::quote<default_delete> {};
...
This proposal uses a trait to customize the behavior. Alternatively, we could have used of overloading a `make_custom` function found by ADL having an additional `id<T>` parameter.
...
The current wording make use of some meta-programming utilities defined in [META].

## General utilities library

***-------------------------------------------------------***
Insert the following Type Traits.
***-------------------------------------------------------***

**20.10.2 Header <experimental/type_traits> synopsis [meta.type.synop]**
...
inline namespace fundamental_v3
{
inline namespace place_holders
{
    // holder type
    struct _t {};
}
    ...
    template <template <class...> class TC >
        struct type_constructor<meta::quote<TC>> : type_constructor<TC<placeholders::_t> > {};
...
*Remark*: This function shall not participate in overload resolution until `meta::is_invokable<TC, meta::deduced_type_t<Xs>...>::value`.
...
*Remark*: This function shall not participate in overload resolution if `meta::is_invokable<TC, meta::deduced_type_t<Xs>...>::value`.
...
There is an implementation at `https://github.com/viboes/std-make/include/experimental/make.hpp`.
...
* Should the resulting *Invokables* high-order function factory `maker` be implementation defined as it is the result of `std::bind`?
